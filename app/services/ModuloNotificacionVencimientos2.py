# -*- coding: utf-8 -*-
"""
PrepararNotificacionesLista (clasificación en ventanas + agrupación por comercial)
-----------------------------------------------------------------------------------------

Clasifica (sin enviar correo ni sellar "Notificaciones") las licencias del pipeline
de Listas en ventanas de vencimiento v30/v60/v90, agrupadas por comercial y por
...
pipeline como la de Excel hayan terminado de clasificar — así cada destinatario
...
Sella el campo "Notificaciones" con 'dd-mm-yyyy-N-OK' (N=3,2,1) según días a vencer:
    ...
    N=2: 30 <= días < 60
    N=3: 60 <= días <= 90
(la función de sellado, SellarNotificacionesLista, la invoca el módulo fusionador).
"""
...
# --------------------- Constantes de campos (defaults) ---------------------
CAMPO_VENCIMIENTO = "Fecha_x0020_Fin"
...
def _clasificar_por_ventana(items: List[Dict[str, Any]], hoy: date, campo_venc: str):
    out = {"v30": [], "v60": [], "v90": []}
    for it in items:
        if not isinstance(it, dict):
            continue
        dias = _days_to_due(it.get(campo_venc), hoy)
        if dias is None:
            continue
        if 0 <= dias <= 30:
            out["v30"].append(it)
...
def _fila(item: Dict[str, Any], campo_venc: str) -> str:
    fecha_corta = _fmt_fecha_corta(item.get(campo_venc))
    return (
        f"<tr>"
        f"<td>{_esc(item.get('id', ''))}</td>"
        f"<td>{_esc(item.get('Consecutivo', ''))}</td>"
        f"<td>{_esc(item.get('Fabricante', ''))}</td>"
        f"<td>{_esc(item.get('Producto', ''))}</td>"
        f"<td>{_esc(item.get('Cantidad', ''))}</td>"
        f"<td>{_esc(fecha_corta)}</td>"
        f"<td>{_esc(item.get('Nombre_x0020_Cliente', ''))}</td>"
        f"<td>{_esc(item.get('Asesor_x0020_ComercialLookupId', ''))}</td>"
        f"</tr>"
    )

def _tabla(items: List[Dict[str, Any]], campo_venc: str) -> str:
    return "\n".join(_fila(it, campo_venc) for it in items)
...
def _contenido_v3090(
        v30: List[Dict[str, Any]],
        v60: List[Dict[str, Any]],
        v90: List[Dict[str, Any]],
        campo_venc: str
    ) -> str:
    badge_30 = (
        '<span style="display:inline-block;padding:2px 8px;'
        'border-radius:12px;font-size:12px;color:#ffffff;'
        'background-color:#d93025;vertical-align:middle;'
        'margin-left:6px;">Prioridad Alta</span>'
    )
    badge_60 = (
        '<span style="display:inline-block;padding:2px 8px;'
        'border-radius:12px;font-size:12px;color:#ffffff;'
        'background-color:#f29900;vertical-align:middle;'
        'margin-left:6px;">Prioridad Media</span>'
    )
    badge_90 = (
        '<span style="display:inline-block;padding:2px 8px;'
        'border-radius:12px;font-size:12px;color:#222222;'
        'background-color:#e5b700;vertical-align:middle;'
        'margin-left:6px;">Seguimiento</span>'
    )

    seccion_30 = _section_html(
        "Vencimiento en menos de 30 días",
        badge_30,
        v30,
        campo_venc
    )
    seccion_60 = _section_html(
        "Vencimiento entre 31 y 60 días",
        badge_60,
        v60,
        campo_venc
    )
    seccion_90 = _section_html(
    ...
    return (
        contenido
        if contenido
        else "<p><em>No se encontraron elementos para mostrar.</em></p>"
    )

def ConstruirFragmentoSeccionesLista(secciones: List[Dict[str, Any]], campo_venc: str) -> str:
    """
    ...
    insertar las tablas de licencias de Listas dentro de un correo combinado junto
    con las de la pipeline de Excel.
    """
    bloques = []
    for sec in secciones:
        t = _esc(sec.get("titulo", ""))
        nota_html = sec.get("nota_html") or ""
        contenido = _contenido_v3090(
            sec.get("v30", []),
            sec.get("v60", []),
            sec.get("v90", []),
            campo_venc
...
def _sellar_notificacion_en_item(
    ...
    *,
    ...
    # --- preservar prefijo (ej: "OK,") si existe ---
    raw_actual = it.get(campo_notif, "") or ""
    prefijo = ""

    if "," in raw_actual:
        # todo lo que esté antes de la primera coma se conserva
        prefijo = raw_actual.split(",", 1)[0].strip()

    # construir solo la parte de notificaciones
    parte_notif = "Notif: " + ", ".join(
        existente[k][1] for k in (3, 2, 1) if k in existente
    )

    # recomponer valor final
    if prefijo:
        it[campo_notif] = f"{prefijo}, {parte_notif}"
    else:
        it[campo_notif] = parte_notif
    return True
...
def SellarNotificacionesLista(
    ...
    """Wrapper público sobre _sellar_en_lote (con la lógica de sellado propia de
    Listas, que preserva un prefijo existente en el campo), para que el módulo
    fusionador no tenga que tocar internals con guion bajo de este módulo."""
...
def PrepararNotificacionesLista(
        CamposEmail: Union[str, List[str], Tuple[str]],
        CampoVencimiento: str,
        CampoNotificaciones: str,
        ListaNotificarLicenciasConEmail: Any,
        ListaNotificarLicenciasSinEmail: Any,
        ListaNotificarLincenciasComercialInactivos: Any,
        LicenciasSinFechaValida: Any,
    ) -> Tuple[bool, str, Dict[str, Any]]:
    """
    Clasifica las licencias del pipeline de Listas en ventanas de vencimiento
    ...
    LicenciasSinFechaValida = _ensure_list_of_dicts(LicenciasSinFechaValida, "FechaNoValida")
    ...
    raw_lists = {
        ...
        "FechaNoValida": LicenciasSinFechaValida,
    }
    ...
    if not (ListaNotificarLicenciasConEmail or ListaNotificarLicenciasSinEmail
            or ListaNotificarLincenciasComercialInactivos or LicenciasSinFechaValida):
        plan = {
            "por_comercial": {},
            "secciones_sin_destinatario": [],
            "secciones_seguimiento_base": [],
            "campo_vencimiento": CampoVencimiento,
            "campo_notificaciones": CampoNotificaciones,
            "raw_lists": raw_lists,
        }
    ...
    clasif_sin = _clasificar_por_ventana(ListaNotificarLicenciasSinEmail, hoy_date, CampoVencimiento)
    clasif_inv = _clasificar_por_ventana(ListaNotificarLincenciasComercialInactivos, hoy_date, CampoVencimiento)
    clasif_con_email = _clasificar_por_ventana(ListaNotificarLicenciasConEmail, hoy_date, CampoVencimiento) if ListaNotificarLicenciasConEmail else {"v30": [], "v60": [], "v90": []}
    ...
    if ListaNotificarLicenciasConEmail:
        ...
        for correo_lower, items in grupos.items():
            clasif_g = _clasificar_por_ventana(items, hoy_date, CampoVencimiento)
    ...
    if LicenciasSinFechaValida:
        secciones_sin_destinatario.append({
            ...
            "v30": LicenciasSinFechaValida, "v60": [], "v90": [],
        })
